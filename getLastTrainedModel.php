...
function getLastTrainedModel($ownerName) {
    $serverName = "137.135.120.162";
    $url = "https://" . $serverName . "/mlp/getLastTrainedModel";
    $ch = curl_init($url);
    $tryTimes = 1;
    $postData = array("ownerName" => $ownerName);
    $httpStatusCode = simpleCurlWithCh($ch, $postData, $response, 10);
    if (isResponseStatusOk($httpStatusCode)) {  // api call failed somehow
	echo "get last trained model!!" . PHP_EOL;
	print_r($response);
	echo  PHP_EOL;
	return;
    } else {
