...
function getMostAccurateModel($modelGroupName, $ownerName) {
    $serverName = "137.135.120.162";
    $url = "https://" . $serverName . "/mlp/getMostAccurateModel";
    $ch = curl_init($url);
    $tryTimes = 1;
    $postData = array("name" => $modelGroupName,
		      "ownerName" => $ownerName);
    $httpStatusCode = simpleCurlWithCh($ch, $postData, $response, 10);
    if (isResponseStatusOk($httpStatusCode)) {  // api call failed somehow
	echo "get most accurate model!!" . PHP_EOL;
	print_r($response);
	echo  PHP_EOL;
	return;
    } else {
...
$ownerName = $argv[2];
getMostAccurateModel($modelGroupName, $ownerName);
